...
def generate_merged_word_cloud(data, stop_words):
    wordcloud = WordCloud(max_words=100, stopwords=stop_words, colormap='YlOrBr', background_color='white').generate(data)
    return wordcloud

@st.cache_data
def generate_specific_word_cloud(year, country, data_dict, stop_words):
    wordcloud = WordCloud(max_words=100, stopwords=stop_words, colormap='YlOrBr', background_color='white').generate(data_dict[(year,country)])
    return wordcloud

@st.cache_data
def display_wordcloud(data_dict, stop_words):
    #  Select the years and country from a dropdown menu
    ...
    if selected_year == 'Select All' and selected_country == 'Select All':
        ...
        wordcloud = generate_merged_word_cloud(merged_values)
